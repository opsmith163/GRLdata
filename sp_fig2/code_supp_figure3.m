%%%%%%%%%%%% Ekman current retrieval
 ...
 load('mycolor.mat');
 %%%% HYCOM magnitude
  filename=strcat('ocn202110010900.nc4');% 10.01  10:05
 ncdisp(filename);
hycomU=ncread(filename,'water_u');
hycomV=ncread(filename,'water_v');
Hdepth=ncread(filename,'depth');
lon=ncread(filename,'lon');
lat=ncread(filename,'lat');
lon=lon-360;

u=fliplr(rot90(hycomU,-1));
v=fliplr(rot90(hycomV,-1));
clear hycomU hycomV
mag=sqrt(u.^2+v.^2);
MAG=mag(:,:,1);
clear u v mag
 ...
 figure('Color',[1 1 1]);
h=imagesc(lon,lat,MAG);set(gca,'YDir','normal');
set(h,'alphadata',~isnan(MAG));
 colormap(rwb2);i=colorbar;
 set(gca,'XTickLabel',{'80W','70W','60W','50W','40W','30W'}); 
 set(gca,'YTickLabel',{'20N','25N','30N','35N','40N','45N','50N','55N','60N'}) 
box on
 hold on
 rectangle('Position',[-60.8 23.2 2 2],'LineWidth',2,'EdgeColor','r');
ylabel(colorbar,'HYCOM velocity (m s^{-1})');
title('(a)','position',[-80,57.04],'FontSize',16,'Color',[0.8500 0.3250 0.0980]);
...
title('(b)','position',[-63.35,25.75],'FontSize',16,'Color',[0.8500 0.3250 0.0980]);
...
set(h,'alphadata',~isnan(wu));
 ...
 set(gca,'YTickLabel',{'23.8N','24N','24.2N','24.4N','24.6N','24.8N','25N','25.2N','25.4N','25.6N','25.8N'}) 
box on
ylabel(colorbar,'zonal wind speed (m s^{-1})');
title('(c)','position',[-63.35,25.75],'FontSize',16,'Color',[0.8500 0.3250 0.0980]);
...
set(h,'alphadata',~isnan(wu));
 ...
 set(gca,'YTickLabel',{'23.8N','24N','24.2N','24.4N','24.6N','24.8N','25N','25.2N','25.4N','25.6N','25.8N'}) 
box on
ylabel(colorbar,'meridional wind speed (m s^{-1})');
title('(d)','position',[-63.35,25.75],'FontSize',16,'Color',[0.8500 0.3250 0.0980]);
...
ylabel(colorbar,'Ekman velocity (m s^{-1})');
title('(e)','position',[-63.35,25.75],'FontSize',16,'Color',[0.8500 0.3250 0.0980]);
...
title('(f)','position',[-0.57,0.2],'FontSize',16,'Color',[0.8500 0.3250 0.0980]);

% title('Validation at 15 m ');


A=getframe(gcf);
imwrite(A.cdata,'sp3f.jpg') %存储调整过大小的图片
